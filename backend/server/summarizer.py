...
CHAT_PROMPT = (
    "다음은 라이브 방송 채팅 로그입니다. 스트리머의 발언을 직접 보진 못했지만, "
    "채팅 내용을 근거로 방송에서 어떤 일이 있었는지 추론해주세요. "
    "'채팅에서', '시청자들이', '~인 것 같아요' 같은 표현은 쓰지 말고, "
    "실제로 그 일이 있었던 것처럼 방송 내용 자체를 직접 서술해주세요. " + _SCHEMA_NOTE +
    "\n\n채팅:\n{content}"
)
...
async def summarize_context(captions_text: str | None, chat_texts: list[str]) -> dict | None:
    """반환: {"source": "captions"|"chat", "bullets": [...]} 또는 내용/LLM이 부족하면 None."""
